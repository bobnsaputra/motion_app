  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Header */}
        <div style={{ padding: '20px 24px 0', borderBottom: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 600, color: '#111' }}>Projects</h2>
          ...
          <div style={{ display: 'flex', gap: 0 }}>
            {(['mine', 'shared'] as const).map(t => (
              ...
              >
                {t === 'mine' ? `My Projects (${projects.length})` : `Shared with me (${sharedProjects.length})`}
              </button>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: '12px 24px 20px', overflowY: 'auto', flex: 1 }}>
          ...
          {/* My Projects tab */}
          {!loading && !error && tab === 'mine' && projects.length === 0 && (
          ...
          {!loading && tab === 'mine' && projects.length > 0 && (
          ...
          {/* Shared with me tab */}
          {!loading && !error && tab === 'shared' && sharedProjects.length === 0 && (
            <div style={{ textAlign: 'center', padding: 32, color: '#9ca3af', fontSize: 14 }}>
              No projects have been shared with you yet.
            </div>
          )}
          {!loading && tab === 'shared' && sharedProjects.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {sharedProjects.map(p => (
                ...
                >
                  <div>
                    <div style={{ fontWeight: 500, fontSize: 14, color: '#111' }}>{p.title}</div>
                    <div style={{ fontSize: 12, color: '#9ca3af', marginTop: 2 }}>
                      by {p.owner_email} · <span style={{ textTransform: 'capitalize' }}>{p.permission}</span>
                    </div>
                  </div>
                </div>
              ))}
