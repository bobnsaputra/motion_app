...
- 01:10 — **Usage-based Free Tier**: Implemented a 24-hour "active usage" limit for free accounts instead of calendar days. Usage is tracked securely via a new `record_usage` RPC and stored in `profiles.usage_seconds_used`. The frontend pulses a heartbeat every 60 seconds *only* when the browser tab is actively visible. (`20260331000000_usage_timer.sql`, `StageBlockingApp.tsx`)
- 01:15 — **Usage UI & Hard Lock**: Added a cryptic countdown timer to the toolbar for free users that turns amber < 1 hour and pulses red < 10 minutes. When usage hits zero, a non-dismissible Upgrade Modal overtakes the screen, hard-locking the app. (`Toolbar.tsx`, `UpgradeModal.tsx`)
- 01:25 — **Password Recovery Flow**: Built a complete 'Forgot Password' flow natively into the Login screen. Requesting a reset sends a Supabase magic link. Upon clicking, `App.tsx` intercepts the `PASSWORD_RECOVERY` event hash and locks the user into an 'Update Password' screen, forcing them to set a new password before seeing the main app. (`Login.tsx`, `App.tsx`)
- 01:30 — **Security: Turnstile Integration**: Integrated Cloudflare Turnstile into `Login.tsx` to stop automated bot signups. Renders natively on all forms (Sign Up, Sign In, Forgot Password) and binds the validation token to every Supabase authentication payload. Added smart reset logic via component `ref` to prevent stale validation tickets when switching auth modes. (`Login.tsx`, `.env`)
- 01:35 — **Email Verification Polish**: Rewrote the signup flow to elegantly handle verified vs unverified sessions. If verification is required, it immediately redirects users to the login screen with a bright green "Check your inbox" success message instead of failing silently. (`Login.tsx`)
- 01:40 — **Resend Verification UI**: Prevented users from getting permanently locked out if their verification link expires. The App actively catches `"Email not confirmed"` errors on login and renders a dynamic `Resend Verification Email` button that triggers `supabase.auth.resend()` with proper routing. (`Login.tsx`)
